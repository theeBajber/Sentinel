...
const TRUSTED_DOMAINS = new Set([
  "google.com", "youtube.com", "googleapis.com", "googleusercontent.com", "gstatic.com",
  "google-analytics.com", "googletagmanager.com", "googleadservices.com", "youtu.be",
  "microsoft.com", "windows.net", "azureedge.net", "office.com", "live.com", "outlook.com",
  "hotmail.com", "msn.com", "skype.com", "bing.com", "microsoftonline.com",
  "apple.com", "icloud.com", "me.com", "apple-dns.net", "cdn-apple.com",
  "amazon.com", "amazonaws.com", "cloudfront.net", "media-amazon.com", "ssl-images-amazon.com",
  "facebook.com", "fbcdn.net", "facebook.net", "fb.com", "instagram.com", "whatsapp.com",
  "twitter.com", "x.com", "t.co", "twimg.com", "linkedin.com", "licdn.com",
  "netflix.com", "nflxext.com", "nflximg.net", "nflxvideo.net",
  "spotify.com", "spotifycdn.com", "scdn.co", "dropbox.com", "dropboxstatic.com",
  "dropboxusercontent.com", "paypal.com", "paypalobjects.com", "github.com",
  "githubassets.com", "githubusercontent.com", "stackoverflow.com", "stack.imgur.com",
  "reddit.com", "redd.it", "redditmedia.com", "wikipedia.org", "wikimedia.org",
  "openai.com", "chatgpt.com", "chat.com", "oaistatic.com", "oaiusercontent.com",
  "anthropic.com", "claude.ai", "poe.com", "pinterest.com", "pinimg.com", "pin.it",
  "tiktok.com", "tiktokcdn.com", "discord.com", "discordapp.com", "discord.gg",
  "vercel.app", "vercel.com", "netlify.app", "netlify.com", "herokuapp.com",
  "cloudflare.com", "workers.dev", "pages.dev", "firebaseapp.com", "firebaseio.com",
  "github.io", "gitlab.io", "medium.com", "substack.com", "hashnode.dev", "dev.to",
  "producthunt.com", "shopify.com", "myshopify.com", "bigcommerce.com",
  "stripe.com", "stripe.network", "jsdelivr.net", "unpkg.com", "cdnjs.cloudflare.com",
...
function deobfuscateHostname(hostname: string): string {
  return hostname
    .replace(/0/g, "o").replace(/1/g, "l").replace(/!/g, "l").replace(/3/g, "e")
    .replace(/4/g, "a").replace(/5/g, "s").replace(/\$/g, "s").replace(/6/g, "b")
    .replace(/7/g, "t").replace(/8/g, "b").replace(/9/g, "g").replace(/@/g, "a")
    .replace(/\\/g, "/").replace(/\|/g, "i").replace(/\+/g, "t").replace(/\(/g, "c")
    .replace(/\)/g, "c").replace(/\[/g, "c").replace(/\]/g, "c")
    .replace(/\{/g, "c").replace(/\}/g, "c").replace(/\?/g, "").replace(/~/g, "")
    .replace(/`/g, "").replace(/"/g, "").replace(/'/g, "");
}

const BRAND_VARIATIONS: Record<string, string[]> = {
  google: ["google.com", "youtube.com", "googleapis.com", "googleusercontent.com", "gstatic.com", "youtu.be"],
  microsoft: ["microsoft.com", "windows.net", "azureedge.net", "office.com", "live.com", "outlook.com", "bing.com"],
  apple: ["apple.com", "icloud.com", "me.com", "cdn-apple.com"],
  amazon: ["amazon.com", "amazonaws.com", "cloudfront.net", "media-amazon.com"],
  facebook: ["facebook.com", "fbcdn.net", "fb.com", "instagram.com", "whatsapp.com"],
  ...
  anthropic: ["anthropic.com", "claude.ai", "poe.com"],
};

const HIGH_RISK_HOSTNAME_KEYWORDS = [
  "malware", "ransomware", "phishing", "exfiltration", "credential", "steal",
  "fraud", "scam", "virus", "trojan", "spyware", "exploit", "botnet", "c2",
  "commandandcontrol", "keylogger",
];

const SUSPICIOUS_KEYWORDS = [
  "secure", "login", "verify", "update", "confirm", "signin", "password",
  "payment", "wallet", "crypto", "account", "reset", "auth", "support",
  "billing", "security", "helpdesk", "ticket", "protect", "safe",
];

const URL_SHORTENERS = ["bit.ly", "tinyurl.com", "t.co", "goo.gl", "ow.ly", "is.gd", "short.link", "tiny.one", "buff.ly"];
const SUSPICIOUS_TLDS = [".tk", ".ml", ".ga", ".cf"];
const CAUTION_TLDS = [".xyz", ".top", ".click", ".download", ".work", ".date", ".party", ".link", ".review", ".country", ".stream", ".fit", ".gdn"];
...
export async function scanUrl(inputUrl: string): Promise<ScanResult> {
  ...
  if (isTrustedDomain(hostname)) {
    return { verdict: "safe", score: 0, reasons: ["Verified trusted domain"], confidence: 100 };
  }
  ...
  // Check database threat patterns
  const threatPatterns = await prisma.threatEntry.findMany({
    select: { id: true, pattern: true, isRegex: true, severity: true },
  });
  ...
  for (const threat of threatPatterns) {
    ...
    if (threat.isRegex) {
      try {
        ...
        matches = regex.test(normalizedUrl) || regex.test(hostname);
      } catch {
        matches = hostname.includes(threat.pattern);
      }
    } else {
      matches = hostname.includes(threat.pattern) || normalizedUrl.includes(threat.pattern);
    }

    if (matches) {
      matchedThreatIds.push(threat.id);
      reasons.push(`Known threat pattern matched: ${threat.pattern}`);
      if (threat.severity === "critical") score += 65;
      else if (threat.severity === "high") score += 50;
      else if (threat.severity === "medium") score += 25;
      else score += 15;
    }
  }

  // IP address used as hostname (suspicious, not unsafe)
  if (/^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$/.test(hostname)) {
    reasons.push("IP address used as hostname");
    score += 25;
  }

  // Non-HTTPS protocol (suspicious)
  if (u.protocol !== "https:") {
  ...
  // Brand impersonation - suspicious but not auto-unsafe
  ...
  // High-risk keywords - suspicious, not unsafe
  ...
  const allLegitimateDomains = Object.values(BRAND_VARIATIONS).flat();
  const isKnownLegitDomain = allLegitimateDomains.some(domain => deobfuscatedHostname === domain);
  if (isKnownLegitDomain && deobfuscatedHostname !== asciiHostname) {
    reasons.push("Typosquatting detected (obfuscated legitimate domain)");
    score += 30;
  }
  ...
  return {
    ...
    threatType: score >= 65 ? "Heuristic Match" : score >= 30 ? "Suspicious Pattern" : undefined,
